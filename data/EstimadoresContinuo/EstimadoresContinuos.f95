...
integer nPuentes,nPuntos,nObservaciones,nPuntosPuentes,nPuntosTR,nEM,i,j,k,l,n
real*8 sigma,alpha,m,delta,x0,m0,sigmaR_hat,mR_hat,alphaR_hat,tol
parameter(nPuentes=100, nPuntos=10001, nObservaciones=101,nEM = 10)
parameter(nPuntosPuentes=(nPuntos-1)/(nObservaciones-1))
parameter(sigma=0.05, alpha=4.5, m=23.0 ,m0=7, x0=0.05,delta = 10.0 / nPuntos, tol=0.002)! sigma menor a 10^-2
...
character(len=100) :: filename = 'EstimadoresContinuo5.csv'
character(len=1)   :: sep = ','
header ='sigma_hat' // sep // 'alpha_hat' // sep //'m_hat'
...
10  call sim_gom(alpha, m, sigma, delta, x0, nPuntos, T)
  ...
  if(((sigmaR_hat /= sigmaR_hat).or.(alphaR_hat /= alphaR_hat)).or.(mR_hat /= mR_hat)) then
    ...
    print*,n,sigmaR_hat,alphaR_hat,mR_hat
    write(1, *) sigmaR_hat,sep,alphaR_hat,sep,mR_hat
  end if
end do
close(unit=1)
end program
